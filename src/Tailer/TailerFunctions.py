...
class Tail():
    ...
    def __init__(self, alignment, gene, rev_comp=False):
        ...
        if not rev_comp:
            ...
            if not alignment.is_reverse: # Positive strand for illumina reads
                read_end = alignment.pos + alignment.reference_length
                self.threeEnd = (read_end) - gene.stop # pysam is zero-indexed, GTFs are 1-indexed
...
def makeTailedReadsDict(alignment_dict, gtf_db, rev_comp):
    ...
    for key, _ in tqdm(alignment_dict.items()):
        ...
        for aln in alignment_dict[key]:
            ...
            for gene in genes:
                out_dict[alignment_dict[key][0].query_sequence].tails.append(Tail(aln, gene,rev_comp = rev_comp))
